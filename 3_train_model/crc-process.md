...
* SLURM file:
        ...
        * OPTIONAL: 
                * To run in cpu, delete all three lines 8, 9, 10 in the slurm script, then change the workon environment to `cpu`
                * Can also change the name of the Python file (`eval.py`) and the out file.
                * Can change it to run on a CPU (make new venv as described above, and see below for changes to slurm file)

If we want to change the partition because the v100 is slow, change the slurm file and Python file:
* Get a list of partitions: `sinfo -M gpu`. Anything that doesn't say 'mix' could be better.
